...
class MockRetriever:
    ...
    def _detect_query_intent(self, query: str) -> str:
        ...
        if any(
            kw in query_lower for kw in ["project", "built", "developed", "created", "worked on"]
        ):
            return "projects"
        ...
        if any(
            kw in query_lower for kw in ["contact", "email", "phone", "reach", "linkedin", "github"]
        ):
            return "contact"
    ...
    def retrieve(self, query: str, k: int = 5, threshold: float = 0.0) -> List[Dict[str, Any]]:
        ...
        for doc in self.documents:
            ...
            if query_intent == "experience":
                ...
                has_work_exp_keywords = any(
                    kw in doc_content_lower
                    for kw in [
                        "work experience",
                ...
                if "work experience" in query_lower:
                    if "work experience" in doc_content_lower:
                        content_bonus += 0.3
                    if (
                        "three professional internships" in doc_content_lower
                        or "three professional" in doc_content_lower
                    ):
                        content_bonus += 0.2
                ...
                results.append(
                    {
                        ...
                        "metadata": doc["metadata"],
                        "score": round(
                            min(score * 1.5, 0.95), 4
                        ),  # Scale up but cap below 1.0 to preserve ranking
                    }
                )
    ...
    def save(self, path: str):
        ...
        state = {"documents": self.documents, "embedding_model": self.embedding_model}
