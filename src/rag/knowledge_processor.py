...
"""
    ...
    return [
        Document(content=content.strip(), metadata={"type": "personal_info", "category": "about"})
    ]
...
def process_skills(data: Dict[str, Any]) -> List[Document]:
    ...
    documents.append(
        Document(content=skills_text.strip(), metadata={"type": "skills", "category": "skills"})
    )
...
"""
    ...
    documents.append(
        Document(content=edu_text.strip(), metadata={"type": "education", "category": "education"})
    )
...
"""
        documents.append(
            Document(
                content=content.strip(),
                metadata={"type": "faq", "question": faq.get("question", ""), "category": "faq"},
            )
        )
...
def process_interests_and_contact(data: Dict[str, Any]) -> List[Document]:
    ...
    if interests:
        ...
        documents.append(
            Document(content=interests_text, metadata={"type": "interests", "category": "about"})
        )
...
"""
        documents.append(
            Document(
                content=contact_text.strip(), metadata={"type": "contact", "category": "contact"}
            )
        )
...
def process_comprehensive_qa(data: Dict[str, Any]) -> List[Document]:
    ...
    for qa_category, qa_list in qa_data.items():
        ...
        for qa in qa_list:
            ...
            if any(
                kw in question_lower or kw in answer_lower
                for kw in [
                    "work experience",
                    "internship",
                    "internships",
...
"""
        documents.append(
            Document(
                content=content.strip(),
                metadata={"type": doc_type, "category": doc_category, "keyword": keyword},
            )
        )
...
"""
        documents.append(
            Document(
                content=content.strip(),
                metadata={"type": "achievement", "category": "about", "area": area},
            )
        )
...
"""
    return [
        Document(content=content.strip(), metadata={"type": "career_goals", "category": "about"})
    ]
...
def process_strengths(data: Dict[str, Any]) -> List[Document]:
    ...
    return [Document(content=content, metadata={"type": "strengths", "category": "about"})]
...
def process_research_interests(data: Dict[str, Any]) -> List[Document]:
    ...
    return [Document(content=content, metadata={"type": "research_interests", "category": "about"})]
...
"""
    return [
        Document(content=content.strip(), metadata={"type": "availability", "category": "about"})
    ]
...
def process_detailed_info(data: Dict[str, Any]) -> List[Document]:
    ...
    for school_key, school_data in edu_extended.items():
        ...
        research_line = f"Research Interests: {research}" if research else ""
        achievements_line = f"Achievements:\n{achievements_text}" if achievements_text else ""
        gpa_note = school_data.get("gpa_note", "")
        gpa_line = f"GPA Note: {gpa_note}" if gpa_note else ""
...
"""
        documents.append(
            Document(
                content=content.strip(),
                metadata={"type": "education", "category": "education", "school": school_key},
            )
        )
...
"""
    ...
    if soft_skills:
        ...
        documents.append(
            Document(
                content=soft_text.strip(), metadata={"type": "soft_skills", "category": "about"}
            )
        )
    ...
    if ideal_env:
        ...
        documents.append(
            Document(
                content=env_text.strip(), metadata={"type": "preferences", "category": "about"}
            )
        )
